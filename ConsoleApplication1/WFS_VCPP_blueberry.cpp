// WFS_VCPP_blueberry.cpp : Diese Datei enthält die Funktion "main". Hier beginnt und endet die Ausführung des Programms.
//
...
ViInt32         hs_win_count_x, hs_win_count_y, hs_win_size_x, hs_win_size_y; // highspeed windows data
ViInt32         hs_win_start_x[MAX_SPOTS_X], hs_win_start_y[MAX_SPOTS_Y];
...
void main(void)
{
	int               err;
	int               i, j, cnt;
	ViInt32               rows, cols;   // image height and width, depending on camera resolution
	int               selection;
	...
	if (selection == 1)
	{
		...
		do
		{
			...
			if ((fopen_s(&fp, SAMPLE_OUTPUT_FILE_NAME, "w")) != NULL)
			{
